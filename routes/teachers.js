...
router.get('/:id', async (req, res) => {
  try {
    ...
    const { rows } = await db.query(
      'SELECT * FROM teachers JOIN users ON teachers.user_id = users.id WHERE id = $1 limit 1',
      [id],
    );
    if (rows.length === 0) {
...
router.put('/update/:id', validateBody(createTeacherSchema), async (req, res) => {
  try {
    const { id } = req.params;
    const { teacher_id, user_id } = req.validatedBody;
    const [rows] = await db.query('SELECT * FROM teachers WHERE id = $1 limit 1', [id]);
    if (rows.length === 0) {
      return res.status(404).json({ message: 'Teacher not found.' });
    }
    await db.query('UPDATE teachers SET teacher_id = $1, user_id = $2 WHERE id = $3', [teacher_id, user_id, id]);
...
router.post('/subjects/create', validateBody(createTeacherSubjectSchema), async (req, res) => {
  try {
    ...
    const { rows } = await db.query('SELECT * FROM teachers WHERE id = $1 limit 1', [teacher_id]);
    if (rows.length === 0) {
      return res.status(404).json({ message: 'Teacher not found.' });
    }
    const { rows: rows1 } = await db.query('SELECT * FROM subjects WHERE id = $1 limit 1', [subject_id]);
    if (rows1.length === 0) {
      return res.status(404).json({ message: 'Subject not found.' });
    }
    const { rows: rows2 } = await db.query(
      'SELECT * FROM teachers_subjects WHERE teacher_id = $1 AND subject_id = ? limit 1',
      [teacher_id, subject_id],
    );
    if (rows2.length > 0) {
      return res.status(400).json({ message: 'Section already assigned to the teacher.' });
    }
    await db.query('INSERT INTO subjects (teacher_id, subject_id, section_id) VALUES ($1, $2, $3)', [
      teacher_id,
      subject_id,
      section_id,
    ]);
...
router.delete('/subjects/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const { rows } = await db.query(
      'SELECT * FROM teachers_subjects WHERE teacher_id = $1 AND subject_id = $1 limit 1',
      [id, id],
    );
    if (rows.length === 0) {
      return res.status(404).json({ message: 'Subject not found.' });
    }
    await db.query('DELETE FROM teachers_subjects WHERE teacher_id = $1 AND subject_id = $1', [id, id]);
    return res.status(200).json({
      success: true,
